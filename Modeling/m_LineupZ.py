...
def LockPointsToAxisZ(finalObj, originalPoints):
    """
    Forces the X and Y of every point on finalObj back to the values
    captured in originalPoints, keeping only the deformed Z value.
    This effectively restricts whatever the Spline Deformer did to
    the Z axis only. originalPoints must be in the same point order
    ...
    for i in range(pointCount):
        ...
        lockedPoints.append(c4d.Vector(original.x, original.y, deformed.z))
...
def main():
    ...
    if NewObj[0].GetType() == c4d.Onull:
        ...
        LockPointsToAxisZ(NewPolyObj, ObjPointsOriginal)
        ...
        # Restrict the deformation to the X axis only
        LockPointsToAxisZ(NewObj[0], ObjPointsOriginal)
