...
def LockPointsToAxisY(finalObj, originalPoints):
    """
    Forces the X and Z of every point on finalObj back to the values
    captured in originalPoints, keeping only the deformed Y value.
    This effectively restricts whatever the Spline Deformer did to
    the Y axis only. originalPoints must be in the same point order
    ...
    for i in range(pointCount):
        ...
        lockedPoints.append(c4d.Vector(original.x, deformed.y, original.z))
...
def main():
    ...
    if NewObj[0].GetType() == c4d.Onull:
        ...
        LockPointsToAxisY(NewPolyObj, ObjPointsOriginal)
        ...
        # Restrict the deformation to the X axis only
        LockPointsToAxisY(NewObj[0], ObjPointsOriginal)
