...
Website:
Version: 1.1.0
Description-US: Align selected elements on Y.
Additional functionality:
  - When Alt is pressed, align to maximum Y value.
  - When Ctrl is pressed, align to minimum Y value.
...
def main():
    ...
    if mode == c4d.Mmodel:
        ...
        # Gather all Y positions from the objects (using absolute/global positions)
        y_values = [obj.GetAbsPos().y for obj in objs]
        
        # Determine alignment value based on modifier keys
        if isAltPressed():
            align_value = max(y_values)
        elif isCtrlPressed():
            align_value = min(y_values)
        else:
            align_value = sum(y_values) / len(y_values)
        
        # Align each object to the computed Y value
        for obj in objs:
            pos = obj.GetAbsPos()
            pos.y = align_value
            obj.SetAbsPos(pos)
    ...
    # Calculate target Y position in world space
    y_values = [p['world_pos'].y for p in all_selected_points]
    ...
    for point_data in all_selected_points:
        ...
        world_target = c4d.Vector(point_data['world_pos'].x, align_value, point_data['world_pos'].z)
        local_pos = matrix_inv * world_target
        all_points[point_data['index']].y = local_pos.y
